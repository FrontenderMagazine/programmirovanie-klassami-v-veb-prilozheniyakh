...
```
[CodePen](http://codepen.io/vyushin/pen/mJBGJL)
...
[CodePen](http://codepen.io/vyushin/pen/KpXxVP)

Код стал более запутанным и еще более сложным для восприятия. Уже на этом этапе можно сказать, что через 2 месяца активной жизни веб-приложения с кодом будет невозможно работать, а главное не приятно. Ведь работа должна доставлять удовольствие. Только представьте, если строк такого кода будет 10.000. К тому же, как поведет себя скрипт, если мы изменим шаблон? Он перестанет работать и нам придется рефакторить весь код.
...
[CodePen](http://codepen.io/vyushin/pen/doVqMZ)
...
Когда мы создаем, например, new Object(), то получаем на вооружение не только сам объект, но и все его свойства и методы. Мы можем создать десятки тысяч экземпляров объекта, а методы и свойства у него всегда будут одинаковые (далее я буду называть это **модель**). Если добавить новый метод в модель, у всех 10.000 экземпляров он появится тоже. Если нам потребуется добавить новое свойство, то экземпляры его унаследуют. Данный подход раскрывает перед нами поистине безграничные возможности. Модель - это сердце веб-приложения.  
...
Как видно из примера, Senior создал 3 продукта через конструкцию new Product(). Имейте ввиду, что Senior не объявлял метод getDiscountedPrice для экземпляров, но тем не менее наши экземпляры содержат их. Теперь firstProduct, secondProduct и thirdProduct также содержат метод getDiscountedPrice.
...
[CodePen](http://codepen.io/vyushin/pen/OVxBPP)
...
Теперь в нашей коллекции содержится 3 товара. Мы можем написать методы для работы с целыми коллекциями данных. Попробуйте самостоятельно доавить в ProductCollection новый метод getProductsByType.

[CodePen](http://codepen.io/vyushin/pen/PqJyWq)
...
![enter image description here](https://api.monosnap.com/rpc/file/download?id=rPIGNuBarRrAUVIBnWwMty92NNcliu)
...
![enter image description here](https://api.monosnap.com/rpc/file/download?id=Eud4AqjezS1Gx2q1zyi8g9Z8zNrB6B)
...
В приложенном архиве есть исходники. Для наглядности я не стал собирать проект в один .js файл, но в боевых условиях конечно нужно собираться.

[seniorDeveloper.zip](http://show.vyushin.ru/frontender.info/programmirovanie-klassami-v-veb-prilozheniyakh/seniorDeveloper.zip)
...
[CodePen](http://codepen.io/vyushin/pen/JdrBMG)

###Расширение нативных классов

Как я уже говорил выше, пользовательскими классы имеют одинаковые права с нативными. Из это следует только одно - мы тоже можем их расширить. Опять же, как мы можем расширить класс?
...
По умолчанию класс String не обладает методом clearing, но мы расширили его через прототип и теперь можем использовать во всех строках. Вообще во всех. По тому что любая строка является всего навсего экземпляром класса String.

[CodePen](http://codepen.io/vyushin/pen/jPGpZQ)
